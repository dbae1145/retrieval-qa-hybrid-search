...
def pinecone_init(pinecone_api_key, pinecone_env, vector_db_name):
    pinecone.init(
...
def indexer(directory, index, embed):
...
def search(query, history, prompt_override, api, model, index):
...
def main():
    st.set_page_config(
        page_title="RetrievalQA",  # Browser tab title
    )
    st.title("RetrievalQA")
    ...
    # if "pinecone_vector_db_name" not in st.session_state:
    #     st.session_state.pinecone_vector_db_name = ''

    with st.sidebar:
        st.markdown("**Introduction**")
        st.markdown("""
        Welcome to RetrivalQA!
        """)

        st.session_state.openai_api_key = st.text_input("**Enter OpenAI API Key**", type="password", placeholder="sk-")
        st.session_state.gpt_model = st.selectbox('Select GPT Model', ('gpt-3.5-turbo', 'gpt4'))
        st.session_state.user_system_prompt = st.text_area("Input Your Own System Prompt")
        st.session_state.pinecone_api_key = st.text_input("**Enter Pinecone API Key**", type="password")
        st.session_state.pinecone_env = st.text_input("**Enter Pinecone Environment Name**")
        st.session_state.pinecone_vector_db_name = st.text_input("**Enter Pinecone Vector DB Name**")
        ...
        if clicked:
            ...
            st.session_state.directory = dirname
        if st.button("Process", key="process_document"):
            with st.spinner("Processing"):
                embed = embedding(st.session_state.openai_api_key)
                index = pinecone_init(st.session_state.pinecone_api_key, st.session_state.pinecone_env, st.session_state.pinecone_vector_db_name)
                indexer(st.session_state.directory, index, embed)
    ...
    if submit_button:
        st.session_state.chat_history = search(query, st.session_state.chat_history,
                                               st.session_state.user_system_prompt, st.session_state.openai_api_key,
                                               st.session_state.gpt_model, st.session_state.vectorstore)
